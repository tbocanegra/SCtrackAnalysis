...
def run_scans(flag):
	cn = fn.replace('001','00'+str(i))
	print('Core ' + str(i) + ': Running swspectrometer inifile'+str(i)+'.ini ' + cn)
	s = open('inifile.ini').read()
	s = s.replace('001','00'+str(i))
	s = s.replace('_ST_','_'+station+'_');
	f = open('inifile'+ str(i) +'.ini','w')
	f.write(s)
	f.close()
	a = subprocess.Popen(['/bin/cat','inifile'+str(i)+'.ini'])#,filename)
	a.wait()
#	a = subprocess.Popen(['./swspectrometer','inifile'+str(i)+'.ini',cn])
